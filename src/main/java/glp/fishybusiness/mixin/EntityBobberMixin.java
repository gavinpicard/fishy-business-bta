...
public abstract class EntityBobberMixin extends Entity {
	...
	@Shadow
	protected void init() {
		this.entityData.define(2, (byte)0);
	}
	...
	/**
	 * @author Gavinsight
	 * @reason Speed up fishing, tiered fishing rods
	 */
	@Overwrite
	public void tick() {
		super.tick();
	...
	public int yoink() {
		...
		if (this.hookedEntity != null) {
			...
			EntityItem entityitem = new EntityItem(this.world, this.x, this.y, this.z, new ItemStack(getCatchItem()));
			dx = this.player.x - this.x;
			dy = this.player.y - this.y;
			dz = this.player.z - this.z;
			distance = (double)MathHelper.sqrt_double(dx * dx + dy * dy + dz * dz);
	...
	@Unique
	private Item getCatchItem() {
		Random random = new Random();

		int chance = random.nextInt(4);
		Item drop = Item.foodFishRaw;
